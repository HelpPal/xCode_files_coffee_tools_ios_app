...
static const CGFloat kDefaultRulerWidth = 1000;
...
static const CGSize  kPointerImageViewSize = {60, 1};
static const CGSize  kPointerImageViewSize1 = {1, 100};
...
- (void)layoutSubviews {
    ...
    if ([self isVertical] == true) {
        ...
        CGAffineTransform transform1 = CGAffineTransformRotate(CGAffineTransformIdentity, rads);
        self.pointerImageView.transform = transform1;
    }
    self.pointerImageView.contentMode = UIViewContentModeScaleAspectFill;
    self.pointerImageView.clipsToBounds = false;
...
- (CGRect)frameForPointerImageView {
    if ([self isVertical] == false) {
        CGFloat pointerImageViewOriginX = (self.frame.size.width - kPointerImageViewSize1.width) / 2;
        CGFloat pointerImageViewOriginY = (self.frame.size.height - kPointerImageViewSize1.height) / 2;
        CGRect rulerCenterViewRect = CGRectMake(pointerImageViewOriginX, pointerImageViewOriginY,
                                                kPointerImageViewSize1.width, kPointerImageViewSize1.height);
        return rulerCenterViewRect;
    }
    else {
        
        CGFloat pointerImageViewOriginY = (self.frame.size.height - kPointerImageViewSize.height) / 2;
        CGRect rulerCenterViewRect = CGRectMake(5, pointerImageViewOriginY,
                                                self.frame.size.width-10, kPointerImageViewSize.height);
        return rulerCenterViewRect;
    }
}
...
- (CGRect)frameForRulerLayer {
    if ([self isVertical] == false) {
        ...
        return CGRectMake(0, value, self.rulerWidth, self.frame.size.height);
    }
}
...
- (void)setupPointerImage {
    ...
    if ([self isVertical] == false) {
        [self.pointerImageView setImage:[UIImage imageNamed:@""]];
    }
    else {
        [self.pointerImageView setImage:[UIImage imageNamed:@"toggleColoured"]];
    }
...
- (void)setupScrollView {
    ...
    self.scrollView.decelerationRate = 0.05;
...
- (void)setupRulerLayer {
    ...
    [[NSUserDefaults standardUserDefaults] setDouble:self.markFrequency forKey:@"markKey"];
    [[NSUserDefaults standardUserDefaults] synchronize];
    self.rulerLayer = [CRRulerLayer new];
    self.rulerLayer.kDefaultFrequencyForMajorMark = _markFrequency;
...
- (void)setRulerWidth:(CGFloat)rulerWidth {
    if ((_rulerWidth != rulerWidth) && (rulerWidth <= 15000) && (rulerWidth > 0)) {
...
- (void)setColor:(UIColor *)color forMarkType:(CRRulerMarkType)markType {
    ...
    if (markType & CRRulerMarkTypeMinor) {
        self.rulerLayer.minorMark.color = [UIColor clearColor];
    }
    [self.rulerLayer setNeedsDisplay];
}
